...
class HFOperations(unittest.TestCase):
    ...
    def test_bmm5(self):
        ...
        s1, s2, s3 = z3.Ints('s1 s2 s3')
        s.add(input == tensor_type.tensor3(D(1, s1), D(1, s2), D(1, s3)))
        self.assertEqual(s.check(), z3.sat)
...
class TestResNet(unittest.TestCase):
    ...
    def test_resnet502(self):
        traced = symbolic_trace(models.resnet50())
        print(compute_flatten_and_reshape(traced))
        for n in traced.graph.nodes:
            n.type = Dyn
        ...
        print(solver.model()[input])
        print('time w/o arithmetic:')
        print(timedelta(seconds=end-start))
...
class TestAlexNet(unittest.TestCase):
    ...
    def test_alexnet3(self):
        alexnet = models.alexnet()
        symbolic_traced : torch.fx.GraphModule = symbolic_trace(alexnet)
        print(compute_flatten_and_reshape(symbolic_traced))
        for n in symbolic_traced.graph.nodes:
            if n.op == 'placeholder':
                n.type = Dyn
        ...
        print('timer w/o arithmetic')
        print(timedelta(seconds=end-start))

        solver.add(v1 > 5)
        assert solver.check() == z3.sat
        end = timer()
        print('timer w arithmetic')
        print(timedelta(seconds=end-start))
